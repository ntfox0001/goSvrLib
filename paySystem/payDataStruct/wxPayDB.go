...
type WxPayBill struct {
	...
	Status         string                      `json:"status" dbdef:"tinyint,update"` // 订单状态
	CreateTime     int64                       `json:"createTime,string" dbdef:"int"` // 订单创建时间
	FinishTime     int64                       `json:"finishTime,string" dbdef:"int"` // 订单完成时间
	procedure1     dbtoolsData.CreateProcedure `dbsql:"create procedure WxPayBill_Finish(inbillId varchar(256), intransactionId varchar(256),instatus tinyint) begin update WxPayBill set transactionId=intransactionId, status=instatus where billId=inbillId;end"`
}
