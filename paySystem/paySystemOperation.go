...
func (*PaySystem) PayRecord_NewBill(userId int, billId string, productId string, fee int, billType string, extentInfo string) error {
	// inuserId int,inbillId varchar(256),inreceiptId text,inproductId varchar(32),intotalFee int,instatus varchar(32),increateTime int,infinishTime int,inextentInfo text
...
// 插入支付记录，一般用于苹果支付等，
// 不需要预先申请id，客户端可以直接发起支付，
// 并返回支付结果，服务器收到结果后，可以通过第三方服务器查询结果
func (*PaySystem) PayRecord_NewPaidBill(userId int, billId string, receiptId string, productId string, fee int, billType string, extentInfo string) error {
	// inuserId int,inbillId varchar(256),inreceiptId text,inproductId varchar(32),intotalFee int,instatus varchar(32),increateTime int,infinishTime int,inextentInfo text
	op := database.Instance().NewOperation("call PayBillTable_Insert(?,?,?,?,?,?,?,?,?,?)",
		userId, billId, receiptId, productId, fee, payDataStruct.PayStatusWaitForVerification, time.Now().Unix(), 0, extentInfo, billType)
...
func (*PaySystem) PayRecord_SetPayStatusSuccess(billId string, receiptId string) error {
	//inbillId varchar(256), inreceiptId varchar(256),instatus varchar(32)
	op := database.Instance().NewOperation("call PayBillTable_PaySuccess(?,?,?)", billId, receiptId, payDataStruct.PayStatusSuccess)
